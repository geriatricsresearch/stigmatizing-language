...
# Data ------------------
boot_file <- 'C:/Users/eespejo/Box/IC Case Study - Julien Cobert/edie/stigmatizing-language/race_boot_patient_v3_sim.parquet'
boot_data <- read_parquet(boot_file)

original_file <- 'C:/Users/eespejo/Box/IC Case Study - Julien Cobert/edie/stigmatizing-language/race_original_v3_sim.parquet'
...
# Violence -------------
pwa_ci[['violence']] <- doPWA(boot_data0_long %>% filter(subclass == 'violence'),
                              boot_data_long %>% filter(subclass == 'violence'))

pwa_diff_ci[['violence']] <- doPWAdiff(boot_data0_long %>% filter(subclass == 'violence'),
                                       boot_data_long %>% filter(subclass == 'violence'))

pwa_ci[['violence']] %>% pwa_plot()
ggsave(paste0(pwa_folder, 'plot-violence.png'), width=10, height=7)  



# Passivity -------------
pwa_ci[['passivity']] <- doPWA(boot_data0_long %>% filter(subclass == 'passivity'),
                              boot_data_long %>% filter(subclass == 'passivity'))

pwa_diff_ci[['passivity']] <- doPWAdiff(boot_data0_long %>% filter(subclass == 'passivity'),
                                       boot_data_long %>% filter(subclass == 'passivity'))

pwa_ci[['passivity']] %>% pwa_plot()
ggsave(paste0(pwa_folder, 'plot-passivity.png'), width=10, height=7)  



# Non-adherence -------------
pwa_ci[['nonadherence']] <- doPWA(boot_data0_long %>% filter(grepl('adher', target_word)),
                               boot_data_long %>% filter(grepl('adher', target_word)))

pwa_diff_ci[['nonadherence']] <- doPWAdiff(boot_data0_long %>% filter(grepl('adher', target_word)),
                                        boot_data_long %>% filter(grepl('adher', target_word)))

pwa_ci[['nonadherence']] %>% pwa_plot()
ggsave(paste0(pwa_folder, 'plot-nonadherence.png'), width=10, height=7)  



# Non-compliance -------------
pwa_ci[['noncompliance']] <- doPWA(boot_data0_long %>% filter(grepl('compli', target_word)),
                                  boot_data_long %>% filter(grepl('compli', target_word)))

pwa_diff_ci[['noncompliance']] <- doPWAdiff(boot_data0_long %>% filter(grepl('compli', target_word)),
                                           boot_data_long %>% filter(grepl('compli', target_word)))

pwa_ci[['noncompliance']] %>% pwa_plot()
ggsave(paste0(pwa_folder, 'plot-noncompliance.png'), width=10, height=7)  


# Non-adherence/compliance -------------
pwa_ci[['noncompli_nonadher']] <- doPWA(boot_data0_long %>% filter(grepl('compli|adher', target_word)),
                                   boot_data_long %>% filter(grepl('compli|adher', target_word)))

pwa_diff_ci[['noncompli_nonadher']] <- doPWAdiff(boot_data0_long %>% filter(grepl('compli|adher', target_word)),
                                            boot_data_long %>% filter(grepl('compli|adher', target_word)))

pwa_ci[['noncompli_nonadher']] %>% pwa_plot()
ggsave(paste0(pwa_folder, 'plot-noncompli_nonadher.png'), width=10, height=7)  
...
pwa_ci[['describe_patient']] <- doPWA(boot_data0_long %>% filter(target_word %in% adjectives_for_patients),
