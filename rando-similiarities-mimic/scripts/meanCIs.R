...
setwd('C:/Users/eespejo/Box/IC Case Study - Julien Cobert/edie/stigmatizing-language/rando-similiarities-mimic/')
...
# Data ------------------
boot_file <- 'C:/Users/eespejo/Box/IC Case Study - Julien Cobert/edie/stigmatizing-language/rando/m3_bootstraps_rando.parquet_fixed'
boot_data <- read_parquet(boot_file)

original_file <- 'C:/Users/eespejo/Box/IC Case Study - Julien Cobert/edie/stigmatizing-language/rando/m3_original_rando.parquet_fixed'
...
# Some words as sampled by me!
# bootstrap_ci0 <- left_join(bootstrap_ci, class_data %>% rename(target_word=base_word))

# bootstrap_ci0 %>% dplyr::filter(target_word %in% some_words) %>% ungroup() %>% facetgrid_plot()
# ggsave(filename=paste0(output_folder, 'facetgrid-rando0.png'), width=7, height=10)
...
set.seed(120)
some_words    <- bootstrap_ci %>% select(target_word) %>% unique() %>% sample_n(30) %>% pull()
bootstrap_ci %>% filter(target_word %in% some_words) %>% facetgrid_plot()
ggsave(filename=paste0(output_folder, 'facetgrid-rando.png'), width=7, height=10)
